...
Type Type::promote() const {
    if(_specifier == CHAR && _indirection == 0 && _declarator == SCALAR) // Promote the char to an int
        return Type(INT, 0);
    else if(_declarator == ARRAY) // Promote an "array of T" to "pointer to T"
        return Type(_specifier, _indirection+1);
    return *this;
}

bool Type::isCompatibleWith(const Type &that) const {
    Type promo_this = promote();
    Type promo_that = that.promote();
    if (promo_this.isNumeric() && promo_that.isNumeric())
        return true;

    else if(promo_this.isPointer() && promo_that.isPointer() && promo_this.specifier() == promo_that.specifier() && promo_this.indirection() == promo_that.indirection())
        return true;

    else if(promo_this.isPointer() && promo_that == Type(VOID, 1))
        return true;

    else if(promo_this == Type(VOID, 1) && promo_that.isPointer())
        return true;

    return false;
}

// Returns if the type is a pointer of T, where T is not void
bool Type::isPointerNotVoid() const {
    return (isPointer() && *this != Type(VOID, 1));
}
